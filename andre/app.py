...
class kamar(BaseModel):
	id = AutoField(primary_key=True)
	lantai = CharField(unique=True)
	nama_kamar = CharField(unique=True)
	harga = IntegerField()
	active = BooleanField(default=True)
...
class transaksi(BaseModel):
	...
	id_kamar = ForeignKeyField(kamar)
...
def create_tables():
	with database:
		database.create_tables([level,user,kamar,transaksi,log])
...
def index():
	if islogin():
		if request.method == 'GET':
			datas = kamar.select().where(kamar.active == True)
			return render_template('index.html',datas=datas)
		else:
			key = request.form['nama_kamar']
			d_kamar = kamar.select().where(kamar.nama_kamar == key)
			if d_kamar.exists():
				return render_template('index.html',datas=d_kamar)
			else:
...
@app.route('/show_kamar')
def show_kamar():
	if isadmin():
		kamars = kamar.select()
		return render_template('show_kamar.html',datas=kamars)
	return redirect(url_for('index'))
...
def tambah_kamar():
	if isadmin():
		if request.method == 'GET':
			...
			d_nama_kamar = request.form['nama_kamar']
			d_harga = request.form['harga']
			d_lantai = request.form['lantai']
			try:
				kamar.create(
					nama_kamar=d_nama_kamar,
					harga=d_harga,
					lantai=d_lantai)
				return redirect(url_for('show_kamar'))
			except:
				return redirect(request.url)
	return redirect(url_for('index'))

@app.route('/edit_kamar/<id_kamar>',methods=['POST'])
def edit_kamar(id_kamar):
	if isadmin():
		d_nama_kamar = request.form['nama_kamar']
		d_harga = request.form['harga']
		d_lantai = request.form['lantai']
		...
		if int(d_active) == 1:
			...
			d_active = False
		try:
			d_kamar = kamar.update(
				nama_kamar=d_nama_kamar,
				harga=d_harga,
				lantai=d_lantai,
				active=d_active).where(kamar.id == id_kamar)
			d_kamar.execute()
			return redirect(url_for('show_kamar'))
		except:
			return redirect(url_for('show_kamar'))
...
@app.route('/delete_kamar/<id_kamar>')
def delete_kamar(id_kamar):
	if isadmin():
		try:
			d_kamar = kamar.delete().where(kamar.id == id_kamar)
			d_kamar.execute()
			return redirect(url_for('show_kamar'))
		except:
			return redirect(url_for('show_kamar'))
...
@app.route('/beli_kamar/<id_kamar>',methods=['POST'])
def beli_kamar(id_kamar):
	if islogin():
		...
		# cek
		d_transaksi = transaksi.select().where((transaksi.selesai == False)&(transaksi.id_kamar == id_kamar))
		if d_transaksi.exists():
			for data in d_transaksi:
				d_checkin_d = datetime.datetime(data.checkin.year,data.checkin.month,data.checkin.day,0,0)
				d_checkout_d = datetime.datetime(data.checkout.year,data.checkout.month,data.checkout.day,0,0)

				checkin_now = datetime.datetime(d_checkin.year,d_checkin.month,d_checkin.day,0,0)
				checkout_now = datetime.datetime(d_checkout.year,d_checkout.month,d_checkout.day,0,0)
		...
		if status == True:
			lama_menginap = datetime.datetime(d_checkout.year,d_checkout.month,d_checkout.day,0,0) - datetime.datetime(d_checkin.year,d_checkin.month,d_checkin.day,0,0)
...
@app.route('/confirm_kamar/<id_trx>')
def confirm_kamar(id_trx):
	if islogin():
...
def penjualan():
	if islogin():
		d_transaksi = transaksi.select().join(user).switch(transaksi).join(kamar)
